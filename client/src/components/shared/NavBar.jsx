...
export default function NavBar() {
  ...
  const navStyle = isMobile
    ? navOpen
      ? { ...S.nav, flexDirection: "column", alignItems: "stretch", gap: 20, padding: "20px 14px", height: "100vh", position: "fixed", left: 0, top: 0, width: "100%", transition: "all 0.3s ease", zIndex: 1000, background: "var(--nav-bg)" }
      : { position: "fixed", top: 10, left: 10, zIndex: 1000, background: "var(--panel-bg)", borderRadius: 8, padding: 5 }
        ...
        <>
          <div style={{ display:"flex", flexDirection: "column", alignItems:"flex-start", gap:18 }}>
            <div style={S.logo} onClick={() => handleNavigate("dashboard")}> 
              ...
              <span style={S.logoTxt}>Voice<span style={{color:"var(--accent)"}}>Box</span></span>
            </div>
            <div style={{ display:"flex", flexDirection: "column", gap:10, width: "100%" }}>
              {links.map(([icon, label, pg]) => (
                <button key={pg} onClick={() => handleNavigate(pg)} style={{
                  ...
                  minHeight: 46,
                  background: page===pg ? "var(--accent-bg)" : "transparent",
                  border:     page===pg ? "1px solid var(--accent-border)" : "1px solid transparent",
                  color:      page===pg ? "var(--accent)" : "var(--text-muted)",
                  lineHeight: 1.15,
                }}>
                  {icon} {label}
                </button>
          ...
          <div style={{ display:"flex", flexDirection: "column", alignItems:"flex-start", gap:8, marginTop: "auto" }}>
            <span style={{ fontSize:11, color:"var(--text-muted)", fontFamily:"monospace" }}>{currentUser.uniqueId}</span>
            <span style={{ ...S.pill, ...rolePillStyle(r) }}>{roleIcon(r)} {ROLE_LABELS[r]?.label || r}</span>
            {r === "school_admin" && <span style={{ fontSize:11, color:"var(--text-muted)" }}>{getSchoolName(currentUser.school)}</span>}
            {r === "dept_admin"   && <span style={{ fontSize:11, color:"var(--text-muted)" }}>{getDeptName(currentUser.department)}</span>}
          </div>
        </>
      )}
    </div>
